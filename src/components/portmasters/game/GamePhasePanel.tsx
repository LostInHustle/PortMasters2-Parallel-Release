...
function MarketPriceReference({ game }: { game: GameState }) {
  return (
    <div className="rounded-xl border border-teal-500/15 bg-teal-500/[0.03] px-3.5 py-2.5 mb-3.5">
      <div className="text-[10px] font-semibold tracking-wide text-muted-foreground/80 mb-1.5">━━ MARKET PRICE REFERENCE (hover for details) ━━</div>
      <div className="flex flex-wrap gap-x-4 gap-y-1">
        {[...RESOURCES, ...PRODUCTS].map((item) => (
          <Term key={item} term={item} content={<ExpectedPriceTooltip price={explainExpectedPrice(game, item)} />}>
            <span className="text-[11px]" style={{ color: COLORS[item] }}>{ICONS[item]} {item}</span>
          </Term>
        ))}
      </div>
    </div>
...
function HarborRoster({ members, ids }: { members: PublicUser[]; ids: string[] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="flex items-center gap-1.5">
        {ids.map((id) => {
          const m = byId.get(id);
          return m ? <Avatar key={id} hue={m.avatarHue} name={m.displayName} size={28} ring /> : null;
        })}
      </div>
      <div className="text-xs text-muted-foreground">{ids.length} captain{ids.length !== 1 ? "s" : ""} in the harbor</div>
    </div>
  );
}
...
function ReadyFooter({
  ...
  if (phaseSync.waiting) {
    return (
      <div className="text-center mt-5 space-y-3">
        <div className="text-sm font-medium text-amber-700 dark:text-amber-300">⏳ Waiting for the rest of the crew…</div>
        <ReadyBar ready={phaseSync.ready} members={members} className="justify-center" />
        <Button variant="secondary" className="rounded-xl" onClick={phaseSync.cancelReady}>
          ↩️ Not ready yet
        </Button>
      </div>
    );
  ...
  return (
    <div className="text-center mt-5">
      <Button className={cn("rounded-xl px-6", idleClassName)} onClick={onConfirm}>
        {idleLabel}
      </Button>
    </div>
  );
...
function Welcome({
  ...
  return (
    <div className="max-w-3xl mx-auto text-center py-4">
      ...
      <p className="text-sm text-muted-foreground mb-4">🌊 Eight Voyages await, become the Sea Master!</p>
      <div className="flex flex-col items-center gap-3 mb-6">
        ...
        {isHost ? (
          <>
            <Button
              size="lg"
              className={cn("rounded-xl h-12 px-8 text-base", canStart && "pm-grad-primary text-white")}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-left max-w-2xl mx-auto">
        <InfoCard tone="emerald" title="🚀 Starting Resources" rows={["📦 Hemp×8, Silk×5, Tea×3", "💰 100 Gold starting funds"]} />
        <InfoCard tone="amber" title="⏱️ Production Delay" rows={["Assign task now → item arrives at Phase 3", "Workers don't produce instantly!"]} />
        <InfoCard tone="sea" title="💸 Round-End Costs" rows={["🔧 Maintenance: 15 Gold (fixed each round)", "👥 Wages settled at Phase 3, not on hire"]} />
        <InfoCard tone="rose" title="🧾 Taxes Explained" rows={["VAT: 5% of finished-good profit margin", "Income Tax: 10% of round net profit"]} />
        <InfoCard tone="amber" title="🏴‍☠️ Pirates & Borrowing" rows={["20% chance of losing all Gold each round", "Hire an escort, or ask the harbor for a loan"]} />
      </div>
      <div className="max-w-2xl mx-auto mt-3 space-y-2">
        <div className="rounded-lg bg-teal-500/[0.06] border border-teal-500/15 px-3.5 py-2.5 text-xs">
          <strong>🔄 4 Phases per Voyage:</strong> 1️⃣ Buy at Ports (+ 🤝 Barter) → 2️⃣ Fill Trade Orders → 3️⃣ Pirates, Wages & Maintenance → 4️⃣ Upgrade Ship
        </div>
        <div className="rounded-lg bg-amber-500/[0.06] border border-amber-500/15 px-3.5 py-2.5 text-xs">
          <strong>💡 New Player Tip:</strong> Rely on raw material orders early. Hire artisans only when you can sustain at least 2 rounds of wages. Always keep funds &gt; Maintenance + All Wages.
        </div>
      </div>
    </div>
  );
...
function BoonDraft({
  ...
  if (phaseSync.waiting) {
    return (
      <div className="max-w-md mx-auto text-center py-10">
        <div className="text-2xl font-bold mb-1 pm-text-gold">🧭 Boon Locked In</div>
        <p className="text-sm text-muted-foreground mb-5">The voyage begins once every captain has chosen.</p>
        <ReadyBar ready={phaseSync.ready} members={members} className="justify-center mb-5" />
        <Button variant="secondary" className="rounded-xl" onClick={phaseSync.cancelReady}>↩️ Choose a different Boon</Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto text-center py-2">
      <div className="text-2xl font-bold mb-1 pm-text-gold">🧭 The Navigator's Compass</div>
      <p className="text-sm text-muted-foreground mb-2">Draft a Boon to synergize with your strategy</p>
      <ReadyBar ready={phaseSync.ready} members={members} className="justify-center mb-3" />
      ...
      >
        {picks.map((b) => (
          ...
          >
            ...
            <div className="font-semibold text-foreground mb-2"><Term term={b.name}>{b.name}</Term></div>
            <div className="text-xs text-muted-foreground leading-relaxed flex-1 mb-4">{b.desc}</div>
            <Button className="pm-grad-gold text-amber-950 font-semibold rounded-xl w-full" onClick={() => phaseSync.markReady((g, l) => selectBoon(g, ctx, b.id, l))}>
              🔒 Lock In Boon
            </Button>
          </motion.div>
        ))}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {game.resourceCards.map((c) => {
          ...
          return (
            <div key={c.id} className={cn("rounded-xl border overflow-hidden flex flex-col", purchased ? "border-emerald-500/30 bg-emerald-500/[0.04]" : "border-black/10 dark:border-white/10 bg-background/50")}>
              <div className="px-3.5 py-2 text-xs font-semibold border-b border-black/5 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.03] flex items-center justify-between">
                <span>📍 {c.port}</span>
                <span className="text-muted-foreground">{c.isProductCard ? "Product" : "Raw Material"}</span>
              </div>
              <div className="p-3.5 flex-1 space-y-1.5">
                {c.resources.map((r, i) => (
                  <div key={i} className="flex items-center text-[12px]">
                    ...
                    <Term term={r.type} content={priceAwareTermContent(game, r.type)}><span className="font-medium" style={{ color: COLORS[r.type] }}>{r.type}</span></Term>
                    <span className="mx-1.5">×{r.quantity}</span>
                    <span className="ml-auto text-muted-foreground">Unit: {r.price}💰</span>
                  </div>
                ))}
                {c.isProductCard && c.resources[0].materialCost ? (
                  <div className="text-[10px] text-muted-foreground pl-6">📦 Mat Cost: {c.resources[0].materialCost} Gold ({c.resources[0].materialDetails})</div>
                ) : null}
                <div className="pt-2 mt-1 border-t border-dashed border-black/10 dark:border-white/10">
                  <Term content={<PriceBreakdownTooltip breakdown={breakdown} />}>
                    <span className="text-rose-600 dark:text-rose-400 font-bold text-sm">💰 Total: {finalCost} Gold</span>
                  </Term>
                  {finalCost < c.totalCost && <span className="text-muted-foreground text-[10px] ml-1">(Was {c.totalCost})</span>}
                </div>
              </div>
              <div className="p-3 pt-0">
                <Button
                  className={cn("w-full rounded-lg", canAfford ? "pm-grad-emerald text-white" : "")}
      ...
      <ReadyFooter phaseSync={phaseSync} members={members} idleLabel="✅ Complete Purchase, Continue" onConfirm={() => phaseSync.markReady((g, l) => completePhase1(g, l))} />
    </div>
  );
}
...
function BarterPhase({
  ...
  const validAmounts = Number.isInteger(offerAmount) && offerAmount >= 1 && Number.isInteger(requestAmount) && requestAmount >= 1;
  const canPost = !sameItem && validAmounts && offerAmount <= owned;

  function submitOffer() {
    if (!canPost) return;
    act((g, l) => { postBarterOffer(g, offerItem, offerAmount, requestItem, requestAmount, l); });
  ...
  function cancelOffer(o: BarterOffer) {
    act((g, l) => { refundBarterOffer(g, o.offerItem, o.offerAmount, l); });
    barter.cancel(o.id);
  }
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="rounded-xl border border-teal-500/15 bg-teal-500/[0.03] p-4 mb-4">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 text-sm">
          ...
          <Button className={cn("rounded-lg", canPost && "pm-grad-emerald text-white")} variant={canPost ? "default" : "secondary"} disabled={!canPost} onClick={submitOffer}>
            🤝 Post Offer
          </Button>
        </div>
        {sameItem && <p className="text-center text-[11px] text-rose-600 dark:text-rose-400 mt-2">Pick two different items to barter.</p>}
        {!sameItem && offerAmount > owned && <p className="text-center text-[11px] text-rose-600 dark:text-rose-400 mt-2">You only have {owned} {offerItem}.</p>}
      </div>
      ...
      {barter.error && (
        <div className="rounded-lg bg-rose-500/10 border border-rose-500/25 px-3.5 py-2 mb-4 text-xs text-rose-600 dark:text-rose-300 flex items-center justify-between">
          ...
          <button onClick={barter.clearError}><X className="h-3.5 w-3.5" /></button>
        </div>
      )}

      <div className="rounded-xl border border-black/10 dark:border-white/10 bg-background/50 p-4 mb-4">
        <h3 className="text-center font-semibold mb-3 text-sm">📋 Open Offers</h3>
        {barter.offers.length === 0 ? (
          <p className="text-center text-xs text-muted-foreground py-4">No offers on the board yet. Be the first.</p>
        ) : (
          <div className="space-y-1.5">
            {barter.offers.map((o) => {
              const mine = o.fromUserId === myUserId;
              const canAfford = getOwnedAmount(game, o.requestItem) >= o.requestAmount;
              return (
                <div key={o.id} className={cn("flex items-center justify-between rounded-md px-3 py-2 text-xs border gap-2", mine ? "bg-amber-500/[0.06] border-amber-500/20" : "bg-background/60 border-black/5 dark:border-white/10")}>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="font-medium">{mine ? "You" : o.fromName}</span>
                    <span className="text-muted-foreground">offer</span>
                    <span style={{ color: COLORS[o.offerItem] }}>{ICONS[o.offerItem]} {o.offerAmount} {o.offerItem}</span>
                    <span className="text-muted-foreground">for</span>
                    <span style={{ color: COLORS[o.requestItem] }}>{ICONS[o.requestItem]} {o.requestAmount} {o.requestItem}</span>
                  </div>
                  {mine ? (
                    <Button size="sm" variant="destructive" className="h-7 px-2.5 text-[10px] rounded shrink-0" onClick={() => cancelOffer(o)}>Cancel</Button>
                  ) : (
                    <Button
                      size="sm"
                      className={cn("h-7 px-2.5 text-[10px] rounded shrink-0", canAfford && "pm-grad-primary text-white")}
                      variant={canAfford ? "default" : "secondary"}
                      disabled={!canAfford}
      ...
      <ReadyFooter phaseSync={phaseSync} members={members} idleLabel="✅ Done Bartering, Continue" onConfirm={() => phaseSync.markReady((g, l) => completeBarterPhase(g, barter.takeMyOpenRefunds(), l))} />
    </div>
  );
}
...
function WorkerMgmt({
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="text-2xl font-bold text-center mb-1">👥 Artisan Management</div>
      <p className="text-center text-sm text-muted-foreground mb-4">💰 Current Funds: {game.money} Gold | 📦 See Inventory on the left</p>
      ...
      <div className="rounded-xl bg-emerald-500/[0.06] border border-emerald-500/20 p-3.5 mb-4 text-xs">
        ...
        <div className="grid grid-cols-4 gap-1.5 mt-2 text-center">
          <div className="pm-grad-emerald text-white rounded-md py-1.5"><div>📋 Now</div><div className="text-[9px] opacity-90">Assign task<br />consume materials</div></div>
          <div className="pm-grad-primary text-white rounded-md py-1.5"><div>🤝 Phase 2</div><div className="text-[9px] opacity-90">Trade orders</div></div>
          <div className="pm-grad-amber text-white rounded-md py-1.5"><div>✅ Phase 3</div><div className="text-[9px] opacity-90">Items produced<br />+ wages paid</div></div>
          <div className="bg-fuchsia-600 text-white rounded-md py-1.5"><div>🚢 Phase 4</div><div className="text-[9px] opacity-90">Shipyard</div></div>
        </div>
        <div className="mt-2 text-emerald-700 dark:text-emerald-300">💡 Materials consumed <strong>now</strong>. Finished goods and wage deductions happen at <strong>Phase 3</strong>, not instantly.</div>
      </div>
      ...
      <div className="rounded-xl border border-teal-500/15 bg-teal-500/[0.03] p-4 mb-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <strong className="text-xs text-teal-700 dark:text-teal-300">Raw Materials:</strong>
            {RESOURCES.map(r => <div key={r} className="flex items-center text-[11px] py-0.5"><span className="mr-1.5">{ICONS[r]}</span><span className="flex-1" style={{ color: COLORS[r] }}>{r}</span><b style={{ color: COLORS[r] }}>{game.inventory[r] || 0}</b></div>)}
          </div>
          <div>
            <strong className="text-xs text-teal-700 dark:text-teal-300">Finished Goods:</strong>
            {PRODUCTS.map(r => <div key={r} className="flex items-center text-[11px] py-0.5"><span className="mr-1.5">{ICONS[r]}</span><span className="flex-1" style={{ color: COLORS[r] }}>{r}</span><b style={{ color: COLORS[r] }}>{game.inventory[r] || 0}</b></div>)}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-amber-500/[0.05] border border-teal-500/15 p-4 mb-4">
        ...
        <div className="text-xs space-y-1 mb-3">
          <div><strong>👩‍🔧 <Term term="Weaver">Weaver</Term></strong>: Linen Clothes(2 Hemp) or Cotton Clothes(2 Hemp+1 Silk), <span className="text-orange-600 dark:text-orange-400">{WAGES.weaver} Gold/round</span></div>
          <div><strong>👩‍🎨 <Term term="Master Weaver">Master Weaver</Term></strong>: Linen, Cotton or Brocade(3 Silk), <span className="text-orange-600 dark:text-orange-400">{WAGES.master} Gold/round</span></div>
          <div><strong>🌸 <Term term="Sachet Maker">Sachet Maker</Term></strong>: Sachet(1 Silk+2 Tea), <span className="text-orange-600 dark:text-orange-400">{WAGES.sachet_maker} Gold/round</span></div>
        </div>
        <div className="flex flex-wrap justify-center gap-2">
          <Button size="sm" className="pm-grad-emerald text-white rounded-lg" onClick={() => act((g, l) => hireWorker(g, "weaver", l))}>
            👩‍🔧 Hire Weaver ({weaverCost}💰/round)
          </Button>
          <Button size="sm" variant="secondary" className="rounded-lg" onClick={() => act((g, l) => hireWorker(g, "master", l))}>
            👩‍🎨 Hire Master ({masterCost}💰/round)
          </Button>
          <Button size="sm" className="pm-grad-amber text-white rounded-lg" onClick={() => act((g, l) => hireWorker(g, "sachet_maker", l))}>
      ...
      {(game.weavers.length || game.masterWeavers.length || game.sachetMakers.length) ? (
        <div className="rounded-xl border border-teal-500/15 bg-teal-500/[0.03] p-4 mb-4">
          <h3 className="text-center font-semibold mb-2">👥 Worker Status & Tasks</h3>
          <WorkerList type="weaver" list={game.weavers} name="Weaver" tasks={["Linen Clothes", "Cotton Clothes"]} act={act} />
          <WorkerList type="master" list={game.masterWeavers} name="Master" tasks={["Linen Clothes", "Cotton Clothes", "Brocade"]} act={act} />
          <WorkerList type="sachet_maker" list={game.sachetMakers} name="Maker" tasks={["Sachet"]} act={act} />
        </div>
      ) : null}

      <ReadyFooter phaseSync={phaseSync} members={members} idleLabel="✅ Complete Management, Set Sail" onConfirm={() => phaseSync.markReady((g, l) => startPhase2(g, ctx, l))} />
    </div>
  );
}
...
function WorkerList({
  ...
  return (
    <div className="my-3">
      <div className="text-xs font-semibold text-teal-700 dark:text-teal-300">{icon} {name}s: {list.length}</div>
      {list.map((w, i) => (
        <div key={i} className="flex items-center justify-between bg-background/60 rounded-md px-3 py-1.5 my-1 text-xs border border-black/5 dark:border-white/10">
          <span>{name} {i + 1}: {w.task ? `Working on: ${w.task}${w.isSkilled ? " (Skilled)" : ""}` : `Idle${w.isSkilled ? " ⭐ Skilled" : ""}`}</span>
          {!w.task && (
            <Button size="sm" variant="destructive" className="h-6 px-2 text-[10px] rounded" onClick={() => act((g, l) => fireWorker(g, type, i, l))}>
              Dismiss ({WAGES[type]}💰)
            </Button>
          )}
        </div>
      ...
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        {tasks.map((t) => {
          ...
          return (
            <Button key={t} size="sm" variant="secondary" className="h-7 px-2.5 text-[10px] rounded" onClick={() => act((g, l) => assignTask(g, type, t, l))}>
              Make {t} (Need {mats})
            </Button>
          );
        })}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {game.customerCards.map((o) => {
          ...
          const hasSilk = o.resources.some(r => ["Silk", "Brocade", "Sachet", "Cotton Clothes"].includes(r.type));
          ...
          if (o.isProductOrder) {
            ...
            vatBreakdown = explainVAT(game, product, o.reward / o.resources[0].required!);
          ...
          return (
            <div key={o.id} className={cn("rounded-xl border overflow-hidden flex flex-col", o.isBrokerFavor ? "border-emerald-500/45 bg-emerald-500/[0.05]" : matchesIntel ? "border-amber-500/40 bg-amber-500/[0.04]" : "border-black/10 dark:border-white/10 bg-background/50")}>
                ...
                {o.resources.map((r, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center text-[12px]">
                      ...
                      <span className="mr-1.5 text-base">{ICONS[r.type]}</span>
                      <Term term={r.type}><span className="font-medium" style={{ color: COLORS[r.type] }}>{r.type}</span></Term>
                      <span className="mx-1.5">×{r.required}</span>
                      <span className="ml-auto text-[10px]" style={{ color: has ? "#10b981" : "#f43f5e" }}>Inv: {game.inventory[r.type] || 0}</span>
                    </div>
                  );
                })}
                <div className="text-[11px] text-rose-600 dark:text-rose-400 mt-1.5">
                  <Term content={<PriceBreakdownTooltip breakdown={transportBreakdown} />}>⚓ Freight: {transport} Gold</Term>
                </div>
              ...
              <div className="p-3 pt-0">
                <Button
                  className={cn("w-full rounded-lg", canComplete && !completed ? "pm-grad-primary text-white" : "")}
      ...
      <ReadyFooter phaseSync={phaseSync} members={members} idleLabel="✅ Complete Trades, Continue" onConfirm={() => phaseSync.markReady((g, l) => completePhase2(g, l))} />
    </div>
  );
}
...
function Settlement({
  ...
  return <SettlementBills game={game} aid={aid} myUserId={myUserId} phaseSync={phaseSync} members={members} />;
}
  ...
  return (
    <div className="max-w-xl mx-auto text-center py-4">
      ...
      <p className="text-sm text-muted-foreground mb-5">
        Before this round's bills come due, your ship has to clear open water. There's a 20% chance pirates find you and take every coin in your hold. Hire an escort to sail through safely, or risk it and save the Gold.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-md mx-auto">
        ...
        >
          <ShieldCheck className="h-5 w-5 mr-2" /> Hire Escort ({escortCost} Gold)
        </Button>
        <Button
          size="lg"
          variant="secondary"
      ...
      <p className="text-[11px] text-muted-foreground mt-3">Escort cost scales with your current Gold ({game.money}), so it's cheapest exactly when you have the least to lose.</p>
    </div>
  );
}
...
function SettlementBills({
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-2xl font-bold text-center mb-4">🔧 Phase 3: Round Settlement</div>

      {game.pirateAttackResolved && (
        <div className={cn("rounded-xl border p-3 my-3.5 text-center text-sm", game.escortHired ? "border-teal-500/20 bg-teal-500/[0.04]" : "border-black/10 dark:border-white/10 bg-background/40")}>
          {game.escortHired ? "🛡️ Escort hired, you sailed through safely this round." : "🌊 You sailed without an escort this round."}
        </div>
      )}

      <div className="rounded-xl bg-amber-500/[0.06] border border-amber-500/20 p-3.5 my-3.5">
        <h3 className="font-semibold text-orange-700 dark:text-orange-300 mb-2">⏳ Bills Due This Round</h3>
        <div className="flex justify-between text-[13px] py-0.5"><span>👥 Worker Wages ({nWorkers} worker{nWorkers !== 1 ? "s" : ""})</span><span className="font-bold">{wagesDue} Gold</span></div>
        <div className="flex justify-between text-sm py-0.5"><span>🔧 Ship Maintenance Fee</span><span className="font-bold">{maintCost} Gold</span></div>
        {game.maintenancePenalty > 0 && <div className="text-[11px] text-muted-foreground pl-2.5">↳ Base {game.fixedCost}g + Overdrive Engine penalty {game.maintenancePenalty}g</div>}
        <div className="flex justify-between text-sm py-0.5 border-t border-orange-500/20 pt-1.5 mt-1.5 font-bold"><span>💸 Total Due</span><span className="text-orange-600 dark:text-orange-400">{totalDue} Gold</span></div>
      </div>
      ...
      <div className="rounded-xl bg-teal-500/[0.04] border border-teal-500/15 p-3.5 my-3.5">
        ...
        <div className="flex justify-between text-[13px] py-0.5"><span>Current Funds</span><span className="text-emerald-600 dark:text-emerald-400 font-bold">{game.money} Gold</span></div>
        <div className="flex justify-between text-[13px] py-0.5"><span>After Settlement</span><span className={cn("font-bold", balanceAfter >= 0 ? "text-teal-700 dark:text-teal-300" : "text-rose-600 dark:text-rose-400")}>{balanceAfter} Gold</span></div>
        <div className="flex justify-between text-[13px] py-0.5"><span>Round Revenue</span><span className="text-emerald-600 dark:text-emerald-400">+{game.roundRevenue} Gold</span></div>
      </div>

      {!canAfford && (
        <div className="rounded-xl border border-rose-500/30 bg-rose-500/[0.04] p-3.5 my-3.5">
          <h3 className="font-semibold text-rose-700 dark:text-rose-300 mb-2 flex items-center gap-1.5"><HandCoins className="h-4 w-4" /> Short on Gold? Ask the Harbor for Help</h3>
          {myRequest ? (
            <div className="flex items-center justify-between text-sm bg-background/50 rounded-lg px-3 py-2">
              <span>🆘 Waiting for a captain to lend you <b>{myRequest.amount} Gold</b>…</span>
              <Button size="sm" variant="destructive" className="h-7 px-2.5 text-[10px] rounded shrink-0" onClick={() => aid.cancel()}>Cancel</Button>
            </div>
          ) : (
            <div className="flex flex-wrap items-center gap-2 text-sm">
              ...
              <Input
                ...
                step={1}
                value={requestAmount}
                onChange={(e) => setRequestAmount(Math.max(1, parseInt(e.target.value, 10) || 1))}
                className="w-20 h-9"
              />
              <span className="text-muted-foreground">Gold from another captain</span>
              <Button size="sm" className="pm-grad-primary text-white rounded-lg" onClick={() => aid.post(requestAmount)}>
                🆘 Request Help
              </Button>
            </div>
          )}
          <p className="text-[11px] text-muted-foreground mt-2">A loan transfers instantly if someone helps. Repay it any time before the voyage ends, or it's deducted automatically at Round 8 and handed to them.</p>
        </div>
      )}

      {otherRequests.length > 0 && (
        <div className="rounded-xl border border-black/10 dark:border-white/10 bg-background/50 p-3.5 my-3.5">
          <h3 className="font-semibold mb-2 text-sm">🆘 Captains Asking for Help</h3>
          <div className="space-y-1.5">
            {otherRequests.map((r) => {
              const canHelp = game.money >= r.amount;
              return (
                <div key={r.id} className="flex items-center justify-between rounded-md px-3 py-2 text-xs border border-black/5 dark:border-white/10 bg-background/60 gap-2">
                  <span><b>{r.fromName}</b> needs <span className="text-rose-600 dark:text-rose-400 font-semibold">{r.amount} Gold</span></span>
                  <Button
                    size="sm"
                    className={cn("h-7 px-2.5 text-[10px] rounded shrink-0", canHelp && "pm-grad-emerald text-white")}
      ...
      {aid.error && (
        <div className="rounded-lg bg-rose-500/10 border border-rose-500/25 px-3.5 py-2 mb-3.5 text-xs text-rose-600 dark:text-rose-300 flex items-center justify-between">
          ...
          <button onClick={aid.clearError}><X className="h-3.5 w-3.5" /></button>
        </div>
      )}
      ...
      <ReadyFooter
        phaseSync={phaseSync}
        members={members}
        idleLabel={canAfford ? `💸 Settle Bills: ${totalDue} Gold` : `⚠️ Force Pay (${game.money}/${totalDue} Gold)`}
...
function Shipyard({
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-2xl font-bold text-center mb-4">🚢 Shipyard & Module Rigging</div>
      <div className="rounded-xl border-2 border-teal-500/20 bg-teal-500/[0.04] p-5 my-4">
        <div className="text-base font-bold text-teal-700 dark:text-teal-300">🚢 Ship Level: {game.shipLevel} | ⚓ Discount: {game.shipLevel * 5} Gold</div>
        <div className="text-sm text-teal-600 dark:text-teal-400 mt-1.5">🔌 Module Slots: {game.equippedModules.length} / {game.shipLevel}</div>
        {game.equippedModules.length ? (
          <div className="mt-3 space-y-1">
            {game.equippedModules.map((m) => <div key={m.id} className="text-xs">{m.icon} <strong><Term term={m.name}>{m.name}</Term></strong>: {m.desc}</div>)}
          </div>
        ) : <div className="text-xs text-muted-foreground mt-2">No modules installed. Upgrade ship to unlock slots!</div>}
      </div>
      {phaseSync.waiting ? (
        <div className="text-center space-y-3">
          <div className="text-sm font-medium text-amber-700 dark:text-amber-300">⏳ Waiting for the rest of the crew…</div>
          <ReadyBar ready={phaseSync.ready} members={members} className="justify-center" />
          <Button variant="secondary" className="rounded-xl" onClick={phaseSync.cancelReady}>↩️ Not ready yet</Button>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {canUpgrade && (
            <Button size="lg" variant={affordable ? "default" : "secondary"} className={cn("rounded-xl", affordable && "pm-grad-primary text-white")} disabled={!affordable} onClick={() => act((g, l) => upgradeShip(g, l))}>
              ⚓ Upgrade Ship (Lvl {game.shipLevel + 1}), Cost {upgCost} Gold | +1 Slot, +5 Discount
            </Button>
          )}
          <Button size="lg" variant={canDraft ? "default" : "secondary"} className={cn("rounded-xl", canDraft && "pm-grad-gold text-amber-950")} disabled={!canDraft} onClick={() => act((g) => { startModuleDrafting(g); })}>
            {slotsFull ? "🔄 Draft & Swap Module (Slots Full)" : "🔧 Draft & Install Module"}
          </Button>
          <Button size="lg" className="pm-grad-emerald text-white rounded-xl" onClick={() => phaseSync.markReady((g, l) => skipUpgrade(g, l))}>
            ⏭️ Continue Voyage
          </Button>
        </div>
      )}
...
function ModuleDraft({
  ...
  return (
    <div className="max-w-4xl mx-auto text-center">
      ...
      <p className="text-sm text-muted-foreground mb-4">Choose a module to install or swap.</p>
      {picks.length === 0 ? (
        <div className="text-center text-muted-foreground text-sm py-8">
          You've already drafted your module choices for this voyage. New options arrive next voyage.
        </div>
      ) : (
        <>
          <div className="flex justify-center mb-4">
          ...
          >
            {picks.map((m, i) => (
              ...
              >
                ...
                <Button className="pm-grad-gold text-amber-950 font-semibold rounded-xl w-full" onClick={() => act((g, l) => handleModuleSelect(g, i, l))}>
                  {game.equippedModules.length < game.shipLevel ? "✅ Install" : "🔄 Swap"}
                </Button>
              </motion.div>
            ))}
          </motion.div>
      ...
      <div className="mt-5">
        <Button variant="secondary" className="rounded-xl" onClick={() => act((g, _l) => { g.phase = 4; })}>⬅️ Back to Shipyard</Button>
      </div>
    </div>
  );
}
...
function ModuleSwap({
  ...
  return (
    <div className="max-w-2xl mx-auto text-center">
      <div className="text-2xl font-bold mb-1 text-rose-600 dark:text-rose-400">🔄 Select Module to Replace</div>
      {newMod && <p className="text-sm text-muted-foreground mb-4">New: {newMod.icon} {newMod.name}: {newMod.desc}</p>}
      <div className="rounded-xl border border-teal-500/15 bg-teal-500/[0.03] p-4 my-4 space-y-2 text-left">
        {game.equippedModules.map((m, i) => (
          <div key={m.id} className="flex justify-between items-center bg-background/60 rounded-md p-2.5 border border-black/5 dark:border-white/10">
            <div>
              <strong>{m.icon} <Term term={m.name}>{m.name}</Term></strong>
              <div className="text-[11px] text-muted-foreground">{m.desc}</div>
            </div>
            <Button size="sm" variant="destructive" className="rounded-lg" onClick={() => act((g, l) => finalizeModuleSwap(g, i, l))}>🗑️ Replace</Button>
          </div>
        ))}
      </div>
      <Button variant="secondary" className="rounded-xl" onClick={() => act((g, _l) => { g.phase = "module_draft"; })}>⬅️ Back to Draft</Button>
    </div>
  );
}
...
function Bankruptcy({ game }: { game: GameState }) {
  return (
    <div className="max-w-md mx-auto text-center py-4">
      ...
      <div className="text-2xl font-bold text-rose-600 dark:text-rose-400 mb-1">Ship Fleet Bankrupt!</div>
      <p className="text-sm text-muted-foreground mb-4">{game.money <= 0 ? "Funds depleted, unable to pay essential operational costs" : "Insufficient funds to cover maintenance and wages"}</p>
      <div className="grid grid-cols-2 gap-2 max-w-sm mx-auto text-left text-sm my-4">
        <div className="text-muted-foreground">🌊 Rounds Completed:</div><div><b>{game.currentRound - 1}/{game.maxRounds}</b></div>
        <div className="text-muted-foreground">💰 Final Funds:</div><div><b>{game.money} Gold</b></div>
        <div className="text-muted-foreground">🏆 Final Reputation:</div><div><b>{game.score}</b></div>
        <div className="text-muted-foreground">🚢 Ship Level:</div><div><b>{game.shipLevel}</b></div>
        <div className="text-muted-foreground">🧾 Taxes Paid:</div><div><b>{game.vatPaid + game.incomeTaxPaid} Gold</b></div>
      </div>
      <div className="rounded-xl border border-teal-500/15 bg-teal-500/[0.04] px-4 py-3 mt-4 text-sm text-muted-foreground">
        Your voyage has ended, but the others are still sailing. Click any captain in the <strong>Harbor Roster</strong> to watch their cargo, workers, and log update live.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="max-w-md mx-auto text-center py-4">
          ...
          {mine?.newMerits.map((meritId) => {
            ...
            return (
              <div key={meritId} className="rounded-xl border-2 border-amber-400 bg-amber-400/10 px-4 py-3 text-center">
                <div className="text-lg font-bold text-amber-600 dark:text-amber-300 flex items-center justify-center gap-2">
                  <MeritIcon id={merit.id} className="h-5 w-5" /> Captain's Merit: {merit.name}
                </div>
      ...
      {isHost ? (
        ...
        <p className="text-sm text-muted-foreground">Waiting for the host to restart the voyage…</p>
      )}
    </div>
  );
}
...
export function GamePhasePanel({ game, ctx, act, members, myUserId, isHost, phaseSync, barter, aid, voyageResult, myLegacy, onRestart, onShowRumors, onShowTutorial }: Props) {
  ...
  function renderPhase() {
    const p = game.phase;
    if (p === 0) return <Welcome phaseSync={phaseSync} members={members} isHost={isHost} onShowTutorial={onShowTutorial} />;
    if (p === 5) return <BoonDraft game={game} ctx={ctx} act={act} phaseSync={phaseSync} members={members} />;
    if (p === 1) return <Purchase game={game} act={act} phaseSync={phaseSync} members={members} onShowRumors={onShowRumors} />;
    if (p === "barter") return <BarterPhase game={game} act={act} barter={barter} myUserId={myUserId} phaseSync={phaseSync} members={members} />;
    if (p === "worker_mgmt") return <WorkerMgmt game={game} ctx={ctx} act={act} phaseSync={phaseSync} members={members} />;
    if (p === 2) return <Orders game={game} act={act} phaseSync={phaseSync} members={members} />;
...
function InfoCard({ tone, title, rows }: { tone: "emerald" | "amber" | "sea" | "rose"; title: string; rows: string[] }) {
  ...
  return (
    <div className={cn("rounded-lg border p-3", tones[tone])}>
      ...
      {rows.map((r, i) => <div key={i} className="text-xs">{r}</div>)}
    </div>
  );
}
