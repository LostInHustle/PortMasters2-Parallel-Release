          ...
          return (
            <Tooltip key={m.id}>
              <TooltipTrigger asChild>
                ...
                >
                  <MeritIcon id={m.id} className={cn("h-3.5 w-3.5", earned ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground")} />
                </span>
              </TooltipTrigger>
              <TooltipContent>
                <div className="font-semibold flex items-center gap-1.5"><MeritIcon id={m.id} className="h-3.5 w-3.5" /> {m.name}</div>
